...
class EditorWindow(PFrame):#(WinFrameBas):
    def __init__(self,RootWindow,ExternalWindow=None,**kwargs):
        #WinFrameBas.__init__(self,RootWindow,**kwargs)
        if ExternalWindow != None:
            self.Particule = RootWindow.Particule
            dicoDefault = {"name":"","geometry":None}
            for index, i in dicoDefault.items():
                if not index in ExternalWindow:
                    ExternalWindow.update({index:i})
            RootWindow = Toplevel(RootWindow.Particule.Mafenetre)
            RootWindow.Particule = self.Particule
            RootWindow.title(ExternalWindow["name"])
            RootWindow.transient=self.Particule.Mafenetre
            if (ExternalWindow["geometry"]!=None):
                RootWindow.geometry(ExternalWindow["geometry"])
        self.RootWindow = RootWindow
        PFrame.__init__(self,RootWindow)
        self.Particule.AllEditorWindow.append(self)
        self.bind("<FocusIn>", self.OnFocus)
        self.bind("<FocusOut>", self.OnLostFocus)
    def destroy(self):
        self.Particule.AllEditorWindow.remove(self)
        PFrame.destroy(self)
